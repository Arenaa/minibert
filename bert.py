...
class BertSelfAttention(nn.Module):
  def __init__(self, config):
    ...
    self.attention_head_size = int(config.hidden_size / config.num_attention_heads) #    hidden_size=768, #attention_head_size = 64
    self.all_head_size = self.num_attention_heads * self.attention_head_size # 768 = hidden_size
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    attention = torch.softmax(energy / (self.attention_head_size ** (1/2)), dim=3)

    # multiply the attention scores to the value and get back V'
    out = torch.einsum("nhql,nlhd->nqhd", [attention, value]).reshape(bs, seq_len, self.num_attention_heads*self.attention_head_size )
    out = self.fc_out(out)
    return out
...
class BertLayer(nn.Module):
  ...
  def add_norm(self, input, output, dense_layer, dropout, ln_layer):
    ...
    out = dropout(ln_layer(input + output))
    return out
    raise NotImplementedError
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    # add-norm layer
    attention_dense = self.attention_dense(attention)
    attention_layer_norm = self.attention_layer_norm(attention_dense + attention)
    attention_dropout = self.attention_dropout(attention_layer_norm)

    # feed forward
    interm_dene = self.interm_dense(attention_dropout)
    feed_forward = self.interm_af(interm_dene)

    # another add-norm layer
    out_dense = self.out_dense(feed_forward)
    out_layer_norm = self.out_layer_norm(feed_forward + out_dense)
    out_dropout = self.out_dropout(out_layer_norm)
